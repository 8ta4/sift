...
function M.clamp_cursor(bufnr, win, opts)
  ...
  local row = math.min(math.max(cursor[1], 1), #state.visible)
  ...
  if opts.force_column or row ~= cursor[1] or cursor[2] < item_text_column then
    pcall(vim.api.nvim_win_set_cursor, win, { row, item_text_column })
  end
end
...
function M.render(bufnr, modified)
  ...
  clamp_buffer_cursors(bufnr, { force_column = true })
end
